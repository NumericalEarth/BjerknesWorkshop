...
# model is composed of: grids, immersed boundaries, forcings, models,
...
# plus the free-surface elevation, and the expensive three-dimensional Poisson solve of the
# nonhydrostatic model is replaced by a two-dimensional problem for the free surface ``\eta``.
...
# ``y``-direction `Flat`, exactly like a two-dimensional turbulence setup but in the vertical
...
# below the bottom height, which is the same approach a realistic regional ocean–sea ice
# simulation uses for coasts and submarine ridges:
...
# a realistic regional ocean–sea ice simulation would replace it with temperature, salinity
# and `SeawaterBuoyancy` with a full equation of state. We use WENO advection for momentum
