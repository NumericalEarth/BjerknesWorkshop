...
#     This tutorial is meant for a GPU machine with internet access. The first run
#     downloads the ECCO4 state estimate, the JRA55-do reanalysis, and the ETOPO1
#     bathymetry (≈ 10 GB altogether, cached locally for every later run). On the
#     workshop cluster the data is pre-staged; on your laptop you can still read along —
#     and run a shortened CPU version by lowering the resolution at the top of the
#     script.
...
#
# One degree of horizontal resolution, 50 vertical levels concentrated near the surface
# by an `ExponentialDiscretization` (the top cell is a few meters thick, the abyssal
# ones a few hundred). Latitude–longitude grids degenerate at the poles — the zonal
# cell width vanishes and with it the affordable time step — so global ocean models
# warp the grid: the `TripolarGrid` places two artificial northern poles over land
...
# (`mutable = true` makes the vertical coordinate a *z★* coordinate that breathes with
# the free surface — relevant for tides and shelf seas, free to keep on.)
#
# The real bathymetry comes from ETOPO1, regridded with a few smoothing passes;
# `major_basins = 2` keeps the two largest connected ocean basins and fills lakes and
# disconnected seas:
...
# ## The ocean component
#
# `ocean_simulation` assembles a `HydrostaticFreeSurfaceModel` configured for realistic
# ocean modeling — temperature and salinity tracers, the TEOS-10 equation of state,
# split-explicit free surface, sensible defaults everywhere — and returns it wrapped in
# a `Simulation`. Every default can be overridden through keyword arguments, with the
...
#
# We initialize ocean and ice from the ECCO4 monthly climatology for January 1993. A
# `MetadataSet` describes *what* to fetch; `set!` regrids it onto our grid, downloading
# (and caching) the data on first use. The same line with `EN4Monthly()` or
# `GLORYSMonthly()` initializes from different products — try it when you wonder how
# much your spin-up remembers its parents:
...
#
# The JRA55-do reanalysis provides the atmospheric state (winds, temperature, humidity,
# precipitation), the downwelling radiation, and land runoff. *Prescribed* means the
# atmosphere does not feel the ocean — the OMIP protocol — but the turbulent fluxes are
...
# ## The coupled model
#
# `OceanSeaIceModel` owns the components and the *interfaces* between them: bulk
# turbulent fluxes at the atmosphere–ocean and atmosphere–ice interfaces, frazil and
# basal melt at the ice–ocean interface, solar penetration, the lot. Every exchanged
# flux is a `Field` you can inspect and output — there is no hidden coupler:
...
# ## Output
#
# Surface fields only, daily — the `indices` trick from the baroclinic channel, at
# planetary scale:
...
# ![](global_ocean.mp4)
#
# Things to look for, with Bjerknes eyes: the Gulf Stream and its (parameterized,
# therefore laminar-looking) North Atlantic Drift feeding the Norwegian Atlantic
# Current; the winter deepening of the marginal ice zone in the Barents Sea; the
# seasonal pulse of the Antarctic ice cover; and the equatorial current system
# responding to the JRA55 wind stress within the first weeks.
#
# ## Where to go from here
#
# - **Resolution.** The same script at `Nx, Ny = 1440, 600` is the quarter-degree,
#   eddy-permitting configuration — provided you also *remove* `eddy_closure` (the
#   resolved eddies object to being parameterized on top). That configuration is the
#   subject of the distributed tutorial next door.
