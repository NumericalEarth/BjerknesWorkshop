...
#
# *Tuesday — one day in the high-latitude ocean, part 5: the calving front.*
#
...
# ## Things to try
#
# !!! tip "The stability threshold"
#     With a squat berg — `width = 180`, ``\epsilon = 0.9`` — the initial tilt *decays*:
#     the berg is metacentrically stable and merely rocks. Hunting the critical aspect
#     ratio by bisection is quite satisfying, and the result lands close to the
#     analytical
#     ``\epsilon_c = \sqrt{6 \frac{\rho_i}{\rho_o}(1 - \frac{\rho_i}{\rho_o})} \approx 0.74``.
#
# !!! tip "Calving into a current"
#     Give the fjord a uniform inflow by initializing `set!(model, u = 0.2)` (and let
#     the default boundary conditions do their imperfect best). The capsize now happens
#     *in* a flow, the wake advects downstream, and the berg drifts as it rolls.
#
# !!! tip "The energy budget"
#     The released potential energy in a full capsize is
#     ``\Delta E = \frac{1}{2} \rho_i g W H (H - W)(1 - \rho_i/\rho_o)`` per unit
#     length. Compute the fluid kinetic energy `Average((u^2 + w^2)/2)` as a time
#     series, and estimate which fraction of ``\Delta E`` ends up in the water column
#     (Burton et al. 2012 measured ~85% dissipated near-field). Repeat with
#     ``\tau`` ten times smaller: where does the energy go now?
#
# !!! tip "Iceberg towing"
#     Add a horizontal towline force to the berg's equation 3 and tow it at 0.5 m s⁻¹
#     through the stratified water: the dead-water resistance problem of Ekman (1904),
#     with a rigid body you implemented yourself.
#
# !!! tip "Real turbulence needs the third dimension (GPU)"
#     However hard you push this setup, the wake stays a collection of filaments and
#     vortex dipoles: in two dimensions there is no vortex stretching, and enstrophy
#     cascades to small scales while the energy goes *up*-scale — yesterday's lecture,
#     working against us today. To watch the wake actually break down: make the domain
#     three-dimensional (`y = (0, 300)`, `topology = (Bounded, Periodic, Bounded)`, the
#     berg spanning `y` uniformly so the mask gains no new arguments), move the grid to
#     `GPU()`, and look for the three-dimensional instability of the shed dipoles. The
#     rigid-body integrals in the callback need a `y`-average — two extra lines.
