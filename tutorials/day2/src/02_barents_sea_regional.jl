...
bottom_height = regrid_bathymetry(underlying_grid;
                                  ...
                                  major_basins = 1, download_keywords...)
...
Tᵉˣᵗ = FieldTimeSeries(Metadata(:temperature;  dates, dataset, region, download_keywords...), grid, inpainting=100, time_indices_in_memory=length(dates))
Sᵉˣᵗ = FieldTimeSeries(Metadata(:salinity;     dates, dataset, region, download_keywords...), grid, inpainting=100, time_indices_in_memory=length(dates))
uᵉˣᵗ = FieldTimeSeries(Metadata(:u_velocity;   dates, dataset, region, download_keywords...), grid, inpainting=100, time_indices_in_memory=length(dates))
vᵉˣᵗ = FieldTimeSeries(Metadata(:v_velocity;   dates, dataset, region, download_keywords...), grid, inpainting=100, time_indices_in_memory=length(dates))
ηᵉˣᵗ = FieldTimeSeries(Metadata(:free_surface; dates, dataset, region, download_keywords...), grid, inpainting=100, time_indices_in_memory=length(dates))
nothing #hide
...
FT = DatasetRestoring(Metadata(:temperature; dates, dataset, region, download_keywords...), grid; rate = 1/1days,     mask = sponge_mask, inpainting=100)
Fu = DatasetRestoring(Metadata(:u_velocity;  dates, dataset, region, download_keywords...), grid; rate = 1/20minutes, mask = sponge_mask, inpainting=100)
Fv = DatasetRestoring(Metadata(:v_velocity;  dates, dataset, region, download_keywords...), grid; rate = 1/20minutes, mask = sponge_mask, inpainting=100)
FS = DatasetRestoring(Metadata(:salinity;    dates, dataset, region, download_keywords...), grid; rate = 1/1days,     mask = sponge_mask, inpainting=100)
...
set!(sea_ice.model, h = Metadatum(:sea_ice_thickness,     date=dates[1], dataset=ECCO4Monthly(), download_keywords...),
                    ℵ = Metadatum(:sea_ice_concentration, date=dates[1], dataset=ECCO4Monthly(), download_keywords...))
