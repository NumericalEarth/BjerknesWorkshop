...
#
# *Tuesday — one day in the high-latitude ocean, part 2: the eddies that carry the heat north.*
#
...
# ## Model
#
# The same `HydrostaticFreeSurfaceModel` as the internal-tide tutorial, plus the drag:
...
simulation.output_writers[:surface] = JLD2Writer(model, (; ζ, b);
...
xζ, yζ, _ = nodes(ζ_timeseries[1])
xb, yb, _ = nodes(b_timeseries[1])
...
Roₙ = @lift interior(ζ_timeseries[$n], :, :, 1) ./ f₀
bₙ  = @lift interior(b_timeseries[$n], :, :, 1)

fig = Figure(size = (1000, 520))
fig[1, :] = Label(fig, title, fontsize = 20, tellwidth = false)

ax_ζ = Axis(fig[2, 1], xlabel = "x [km]", ylabel = "y [km]",
            title = "surface vorticity ζ/f", aspect = 1)
hm_ζ = heatmap!(ax_ζ, xζ ./ 1e3, yζ ./ 1e3, Roₙ, colorrange = (-0.4, 0.4), colormap = :balance)
Colorbar(fig[2, 2], hm_ζ)

ax_b = Axis(fig[2, 3], xlabel = "x [km]", ylabel = "y [km]",
            title = "surface buoyancy", aspect = 1)
hm_b = heatmap!(ax_b, xb ./ 1e3, yb ./ 1e3, bₙ, colormap = :deep)
...
# parameterization mimics in coarse global configurations — and that the Barents Sea simulation of part 6 will resolve live.
#
# ## Things to try
#
# !!! tip "Resolution and the eddy scale"
#     Double the horizontal resolution to (192, 192) — on the GPU if you have one,
#     `RectilinearGrid(GPU(); ...)` being the only change. Compare the vorticity field:
#     which scales appear? Does the equilibrated kinetic energy change?
#
# !!! tip "The deformation-radius dial"
#     Halve the stratification `N²`. The deformation radius shrinks by ``\sqrt{2}`` —
#     verify that the emerging eddies do too, and check the effect on the growth phase
#     of the kinetic energy curve.
#
# !!! tip "The eddy heat flux"
#     Add an output for the meridional buoyancy flux `Average(v * b, dims=(1, 3))` and
#     plot its profile across the channel in the equilibrated phase. This profile is
#     exactly the object that ``\kappa_{skew}`` models in coarse simulations.
#
# !!! tip "Adding a wind"
#     Turn the spin-down into a forced-dissipated equilibrium by adding a westerly wind
#     stress at the surface,
#     `FluxBoundaryCondition((x, y, t, p) -> -p.τ₀ / p.ρₒ * cos(π * y / p.Ly)^2,
#     parameters = (τ₀ = 0.05, ρₒ = 1026, Ly = Ly))` as the `top` boundary condition on
#     `u`. With no vertical mixing closure the momentum stays in the top grid cell —
#     look at the surface jet that develops, then add
#     `closure = VerticalScalarDiffusivity(ν = 1e-3)` to the model and compare. There is
#     a lesson in this: boundary conditions and closures are partners, not strangers.
